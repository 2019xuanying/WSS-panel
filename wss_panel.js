 ...
 * - [架构] 数据流反转 (Pull -> Push):
 * - Panel (控制平面) 现在被动接收来自 Proxy (数据平面) 的
 * `{ type: 'stats_update' }` 实时推送 (每 1 秒)。
 * - [需求 #3] 动态频率 (在线/离线):
 * - 新增 "/ws/ui" WebSocket 路径，供前端连接。
 * - 新增 `wssUiPool` (Set) 来跟踪在线的管理员 UI。
 * - `stats_update` 处理器检查 `wssUiPool.size > 0`。
 * - 如果 > 0 (在线)，立即聚合数据并推送 `live_update` 到前端 (1秒实时)。
 * - 如果 = 0 (离线)，不推送，只在后台累积流量。
 * - [需求 #2] 熔断 Bug 修复 (Part 2):
 * - `stats_update` 处理器现在是唯一的熔断决策者。
 * - 当收到超速数据时，它会立即将用户 `fused`、锁定 (`usermod -L`)
 * 并 `kickUserFromProxy`。这是一个原子操作。
 * - [需求 #3] 60秒维护:
 * - `BACKGROUND_SYNC_INTERVAL` 更改为 60000ms (60秒)。
 * - `syncUserStatus` 被“降级”，不再 fetch /stats API。
 * - `syncUserStatus` 现在只负责处理慢速状态 (到期/超额)
 * 和推送系统状态 (CPU/内存)。
 * - [架构] API 清理:
 * - 移除了 `/api/users/live-stats` (已废弃)。
 * - 移除了 `getProxyLiveConnections` (已废弃)。
 * - `getSystemStatus` 不再调用 /stats API，
 * 实时连接数现在来自 `workerStatsCache`。
 */
